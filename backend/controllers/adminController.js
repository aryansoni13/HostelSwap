...
exports.decreaseRooms = async (req, res) => {
  try {
    ...
    const roomsOfType = hostelData.rooms.filter(room => room.bedType === bedType);
    ...
    const updatedRooms = hostelData.rooms.filter(room => !roomNumbersToRemove.includes(room.roomNumber));
