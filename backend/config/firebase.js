...
try {
  ...
  if (admin.apps.length === 0) {
    // Option 0: Cloud Functions / Google Cloud Environment (Auto-discovery)
    if (process.env.FIREBASE_CONFIG || process.env.K_SERVICE) {
      admin.initializeApp();
      console.log(
        "Firebase Admin initialized automatically (Cloud Environment)"
      );
    }
    ...
    // Option 3: Try to use default service account file location
      ...
      try {
        ...
        console.error(
          "   OR set FIREBASE_SERVICE_ACCOUNT environment variable with JSON content\n"
        );
        throw new Error(
          "Firebase Admin SDK credentials not found. Please set up service account key."
        );
      }
    }
  }
